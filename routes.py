...
def remove_item(user_id, item_id):
	...
	# commit the deletion
	removing_item = Item.query.get(item_id)
	db.session.delete(removing_item)
	db.session.commit()
	return redirect(url_for('profile', user_id=user_id))
...
def dashboard():
	...
	top_5= popular_songs[:5]

	songs = Song.query.all()
	return render_template('dashboard.html', songs=songs, popular_songs=top_5, form=form)