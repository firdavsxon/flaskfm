...
# creating the Song model here + add a nice representation method
class Song(db.Model):
	id = db.Column(db.Integer, primary_key=True)
...
class Item(db.Model):
	...
	def __repr__(self):
		return f"{Song.query.get(self.song_id).artist} - {Song.query.get(self.song_id).title}"
...
class Playlist(db.Model):
	...
	def __repr__(self):
		return f'{Item.playlist_id} {Item.song_id}'
